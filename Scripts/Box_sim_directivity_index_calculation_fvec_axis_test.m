...
figure(6)
fig6 = semilogx(frequency,DI_matrix{2},'-o',...
    frequency,DI_matrix{3},'-o',frequency,DI_matrix{4},'-o',frequency,DI_matrix{1},'-o',...
    [frequency(1466) frequency(1466) frequency(1466) frequency(1466)],...
    [DI_matrix{2}(1466) DI_matrix{3}(1466) DI_matrix{4}(1466) DI_matrix{1}(1466)],...
    '*k');
set(fig6,'Linewidth',1)
fig6(1).Color = [0 0.4470 0.7410]; %blue
...
labels = {'Max. deviation in c','0.2° off normal incidence',...
    'Max. dev. in box dimensions','Original simulation'};
neworder = [4 1 2 3]; % Re-order Legend
g = legend(fig6(neworder), labels(neworder));
set(g,'Location','best')
set(gcf,'color','w')


figure(7)
fig8 = semilogx(frequency,(diff1_best_index),'-',frequency,...
    (diff2),'-',frequency,(diff3_best_index),'-');
set(fig8,'Linewidth',1)
%fig6(1).Color = [0 0.4470 0.7410]; %blue
grid on
%xlim([250 20000])
